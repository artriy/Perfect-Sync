...
export function Toggle({ on, onChange, disabled, label }: ToggleProps) {
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onChange}
      className="native-toggle ring-focus relative h-[22px] w-[40px] shrink-0 rounded-full transition-colors disabled:opacity-50"
      style={{
        background: on
    ...
    >
      <motion.span
        aria-hidden="true"
        className="native-toggle-knob absolute top-[3px] left-[3px] h-4 w-4 rounded-full bg-white shadow"
        animate={{ x: on ? 18 : 0 }}
